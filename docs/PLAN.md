...
## 다음 액션
Phase 1~4 모두 완료, Gomoku는 실제 학습 검증까지 끝남 (`checkpoints_gomoku/best.pt`).
다음 선택지: (a) Gomoku를 더 오래/크게 학습시켜 더 강하게 (지금도 계속 반복 늘리면
좋아지는 추세), (b) Deep Sea Crew도 gatekeeping을 도입해서 같은 방식으로 재학습 —
1차 실험에서 학습 신호가 안 보였던 게 어쩌면 여기도 같은 원인(승격 없이 무조건
덮어쓰기)이었을 수 있음, (c) 실제 룰북 확보해 `data/deep_sea_crew/tasks.json`을
진짜 96장 과제로 교체, (d) 웹 UI/UX 다듬기 (재연결, 방 목록, React 전환 등).

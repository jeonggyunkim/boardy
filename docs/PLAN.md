...
## 다음 액션
Phase 1~4 모두 완료, Gomoku는 실제 학습 검증 + 렌주룰까지 끝남
(`checkpoints_gomoku/best.pt`). 다음 선택지: (a) 렌주룰 하에서 Gomoku를 재학습시켜
렌주 전술까지 익히게 하기, (b) Gomoku를 더 오래/크게 학습시켜 더 강하게, (c) Deep
Sea Crew도 gatekeeping을 도입해서 같은 방식으로 재학습 — 1차 실험에서 학습 신호가
안 보였던 게 어쩌면 여기도 같은 원인(승격 없이 무조건 덮어쓰기)이었을 수 있음,
(d) 실제 룰북 확보해 `data/deep_sea_crew/tasks.json`을 진짜 96장 과제로 교체,
(e) 웹 UI/UX 추가 다듬기 (재연결, 방 목록, React 전환 등).

...
## 다음 액션
Deep Sea Crew는 이번 라운드로 규칙이 사용자 검토를 거쳐 안정화됨 (드래프트, 통신
타이밍, 조기 실패 판정 모두 수정 완료). Gomoku는 15×15로 바뀌었고 재학습은 보류
중. 다음 선택지: (a) 15×15 기준으로 Gomoku 재학습 (사용자가 다시 요청할 때),
(b) Deep Sea Crew도 gatekeeping을 도입해서 같은 방식으로 재학습 — 1차 실험에서
학습 신호가 안 보였던 게 어쩌면 여기도 같은 원인(승격 없이 무조건 덮어쓰기)이었을
수 있음, (c) 실제 룰북 확보해 `data/deep_sea_crew/tasks.json`을 진짜 96장 과제로
교체, (d) 웹 UI/UX 추가 다듬기 (재연결, 방 목록, React 전환 등).
